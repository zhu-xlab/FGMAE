...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        if torch.cuda.is_available()==True:
            self.hog.cuda()
        ## indices
        self.decoder_pred2 = nn.Linear(decoder_embed_dim, patch_size**2 * 3, bias=True) # decoder to patch
        '''
        ### define features
        if self.feature=='raw':
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        x1 = self.decoder_pred(x)
        x2 = self.decoder_pred2(x)
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        pred,pred2 = self.forward_decoder(latent, ids_restore)  # [N, L, XXX]
        ...
        target_1d2 = self.patchify(indices)
        target_2d2 = indices        
        
 
        loss1 = self.forward_loss(target_1d, pred, mask)
        loss2 = self.forward_loss(target_1d2, pred2, mask)
        loss = loss1 + loss2
        return loss, mask, pred, target_2d, pred2, target_2d2
