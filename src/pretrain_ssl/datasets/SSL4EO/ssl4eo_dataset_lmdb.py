...
class LMDBDataset(Dataset):
    ...
    def __init__(self, lmdb_file, is_slurm_job=False, s1_transform=None, s2a_transform=None, s2c_transform=None, subset=None, normalize=False, mode=['s1','s2a','s2c'], dtype='uint8'):
    ...
    def __getitem__(self, index):
        ...
        if self.mode==['s1']:
            ...
            if self.dtype=='uint8':
                ...
                ## normalize, percentile maybe necessary to pre-process raw SAR data
                self.max_q = np.quantile(sample_s1.reshape(-1,2),0.99,axis=0) # VV,VH       
                self.min_q = np.quantile(sample_s1.reshape(-1,2),0.01,axis=0) # VV,VH
                img_seasons = []
                for s in range(4):
                    img_bands = []
                    for b in range(2):
                ...
                sample_s1 = np.stack(img_seasons,axis=0) # [4,2,264,264]
            if self.s1_transform is not None:
                sample_s1 = np.transpose(sample_s1[season1],(1,2,0))
        ...
        if self.mode==['s2a']:
            ...
            if self.s2a_transform is not None:
                sample_s2c = np.transpose(sample_s2c[season1],(1,2,0))
