...
from datasets.BigEarthNet.bigearthnet_dataset_official_lmdb_s2_int16 import LMDBDataset,random_subset
...
def get_args_parser():
    ...
    # Augmentation parameters -- not used 
    ...
    parser.add_argument("--normalize", action='store_true', default=False, help="normalize input or not")
...
def main(args):
    ...
    dataset_train = LMDBDataset(
        ...
        #lmdb_file=os.path.join(args.data_path, 'seco_split/train_B12.lmdb'),
    ...
    if args.eval:
        ...
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc_micro']:.2f}% {test_stats['acc_macro']:.2f}% {test_stats['f1_micro']:.2f}% {test_stats['f1_macro']:.2f}%")
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch==args.start_epoch or epoch%5==0 or (epoch + 1 == args.epochs):
            test_stats = evaluate(data_loader_val, model, device, criterion)
