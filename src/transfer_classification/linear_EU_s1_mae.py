...
from datasets.EuroSat.eurosat_dataset_s1 import EurosatDataset,Subset
...
def main(args):
    ...
    eurosat_dataset = EurosatDataset(root=args.data_path,bands='B2',normalize=True)
    ...
    if not args.fine_tune:
        ...
        optimizer = torch.optim.SGD(model_without_ddp.parameters(), args.lr,
                                    momentum=0.9,
                                    weight_decay=args.weight_decay)        
    print(optimizer)
    ...
    if args.eval:
        ...
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}% {test_stats['aAcc']:.1f}%")
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if log_writer is not None:
            ...
            log_writer.add_scalar('perf/test_aAcc', test_stats['aAcc'], epoch)
