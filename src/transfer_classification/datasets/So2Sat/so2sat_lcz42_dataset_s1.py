...
# rVH, iVH, rVV, iVV, VH, VV, rCOV, iCOV
SO2SAT_MEAN = [0, 0,  0,  -20.26,  -12.59,  0,  0,  0]
SO2SAT_STD = [1, 1, 1, 5.91, 5.26, 1, 1, 1]
...
class So2SatDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for b in range(8):
            if b==4 or b==5:
                ch = patch[:,:,b]
                ch_db = 10 * np.log10(ch)
                p1,p99 = np.quantile(ch_db,(0.01,0.99))
                ch_db = np.clip(ch_db, p1, p99)
                ch_db = normalize(ch_db,SO2SAT_MEAN[b],SO2SAT_STD[b])
                channels.append(ch_db)
            else:
                continue        
        
        sample = np.dstack((channels[1],channels[0]))
