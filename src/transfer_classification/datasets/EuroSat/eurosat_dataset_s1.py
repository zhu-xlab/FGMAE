...
### SSL4EO stats
'''
# not ssl4eo, maybe BE, anyway s2 stats not used in eurosat transfer
BAND_STATS = {
    'mean': {
        'B01': 1353.72696296,
        'B02': 1117.20222222,
    ...
    def __getitem__(self, index):
        ...
        with rasterio.open(path) as f:
            if self.bands == ALL_BANDS:
                ...
                array = f.read().astype(np.float32) # vv,vh
                            
            img = array.transpose(1, 2, 0)

        channels = []
        
        for i,b in enumerate(self.bands): # vv,vh
            ch = img[:,:,i]
            
            if self.bands == S1_BANDS:
                max_q = np.quantile(ch,0.99)      
                min_q = np.quantile(ch,0.01)            
                ch[ch>max_q] = max_q
                ch[ch<min_q] = min_q
                ch = normalize(ch, mean=BAND_STATS_S1['mean'][b], std=BAND_STATS_S1['std'][b]) # vv,vh
            else:
                if self.normalize:
...
class Subset(Dataset):
    ...
    def __getitem__(self, idx):
        im, target, path = self.dataset[self.indices[idx]]
        if self.transform:
